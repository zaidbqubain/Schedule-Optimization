...
/* check whether using a unique student id or not */
        $sql = "SELECT * from users where student_id = (:student_id) And id !=(:editid);";
        $qry = $dbh -> prepare($sql);
        $qry-> bindParam(':student_id', $student_id, PDO::PARAM_STR);
		$qry-> bindParam(':editid', $editid, PDO::PARAM_STR);
        $qry->execute();
	...
	$sql="UPDATE users SET name=(:name), email=(:email), student_id=(:student_id),department_id=(:department_id), gender=(:gender), designation=(:designation), Image=(:image) WHERE id=(:idedit)";
...
<div class="form-group">
	<?php if(htmlentities($result->role)=="student") { ?>
<label class="col-sm-2 control-label">Student ID</label>
<div class="col-sm-4">
<input type="text" name="student_id" class="form-control" value="<?php echo htmlentities($result->student_id);?>">
</div>
<?php } ?>
<label class="col-sm-2 control-label">Role</label>
<div class="col-sm-4">
<input type="text" name="role" class="form-control" value="<?php echo htmlentities($result->role);?>" disabled>
</div>
</div>
	...
	<select name="department_id" class="form-control" required>
		<option value="">Select</option>
	    	<?php  foreach($departments as $department){ ?>
		     <option value="<?php echo $department['id']; ?>" <?php if(htmlentities($result->department_id)==$department['id']) { echo "selected"; } ?>><?php echo $department['name']; ?></option>
		   <?php } ?>                            
		</select>
